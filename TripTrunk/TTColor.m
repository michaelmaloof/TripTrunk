//
//  TTColor.m
//  TripTrunk
//
//  Created by Michael Cannell on 3/16/16.
//  Copyright © 2016 Michael Maloof. All rights reserved.
//

#import "TTColor.h"

@implementation TTColor

+ (UIColor *)tripTrunkBlueLinkColor {
    return [UIColor colorWithRed:(142.0/255.0) green:(211.0/255.0) blue:(253.0/255.0) alpha:1];
}

+ (UIColor *)subPhotoPink {
    return [UIColor colorWithRed:(217.0/255.0) green:(135.0/255.0) blue:(135.0/255.0) alpha:1];
}

+ (UIColor *)subPhotoTan {
    return [UIColor colorWithRed:(221.0/255.0) green:(203.0/255.0) blue:(178.0/255.0) alpha:1];
}

+ (UIColor *)subPhotoGreen {
    return [UIColor colorWithRed:(174.0/255.0) green:(221.0/255.0) blue:(177.0/255.0) alpha:1];
}

+ (UIColor *)subPhotoBlue {
    return [UIColor colorWithRed:(118.0/255.0) green:(164.0/255.0) blue:(184.0/255.0) alpha:1];
}

+ (UIColor *)subPhotoGray {
    return [UIColor colorWithRed:(78.0/255.0) green:(79.0/255.0) blue:(85.0/255.0) alpha:1];
}

+ (UIColor *)tripTrunkBlue {
    return [UIColor colorWithRed:(142.0/255.0) green:(211.0/255.0) blue:(253.0/255.0) alpha:1];
}

+ (UIColor *)tripTrunkRed {
    return [UIColor colorWithRed:(255.0/255.0) green:(102.0/255.0) blue:(102.0/255.0) alpha:1];
}

+ (UIColor *)tripTrunkRedThirdAlpha {
    return [UIColor colorWithRed:255.0/255.0 green:102.0/255.0 blue:102.0/255.0 alpha:.3];
}

+ (UIColor *)tripTrunkGray {
    return [UIColor colorWithRed:(194.0/255.0) green:(196.0/255.0) blue:(198.0/255.0) alpha:1];
}

+ (UIColor *)tripTrunkYellow {
    return [UIColor colorWithRed:(255.0/255.0) green:(233.0/255.0) blue:(106.0/255.0) alpha:1];
}

+ (UIColor *)tripTrunkWhite {
    return [UIColor colorWithRed:(255.0/255.0) green:(255.0/255.0) blue:(255.0/255.0) alpha:1];
}

+ (UIColor *)tripTrunkWhiteTransparent {
    return [UIColor colorWithRed:(255.0/255.0) green:(255.0/255.0) blue:(255.0/255.0) alpha:0.95];
}

+ (UIColor *)tripTrunkLightBlue {
    return [UIColor colorWithRed:(131.0/255.0) green:(226.0/255.0) blue:(255.0/255.0) alpha:1];
}

+ (UIColor *)tripTrunkLightGray {
    return [UIColor colorWithRed:(205.0/255.0) green:(205.0/255.0) blue:(205.0/255.0) alpha:1];
}

+ (UIColor *)tripTrunkTurquoise {
    return [UIColor colorWithRed:(91.0/255.0) green:(237.0/255.0) blue:(255.0/255.0) alpha:1];
}

+ (UIColor *)tripTrunkOffWhite {
    return [UIColor colorWithRed:250.0/255.0 green:244.0/255.0 blue:229.0/255.0 alpha:1.0];
}

+ (UIColor *)tripTrunkBlack {
    return [UIColor colorWithRed:0.0/255.0 green:0.0/255.0 blue:0.0/255.0 alpha:1.0];
}

+ (UIColor *)tripTrunkDarkGray {
    return [UIColor colorWithRed:76.0/255.0 green:76.0/255.0 blue:76.0/255.0 alpha:1.0];
}

+ (UIColor *)tripTrunkClear {
    return [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:0.0];
}

+ (UIColor *)tripTrunkGreen {
    return [UIColor colorWithRed:0.0/255.0 green:179.0/255.0 blue:0.0/255.0 alpha:1.0];
}

+ (UIColor *)tripTrunkButtonTextBlue {
    return [UIColor colorWithRed:78.0/255.0 green:179.0/255.0 blue:252.0/255.0 alpha:1.0];
}

+ (UIColor *)tripTrunkBackgroundLightBlue {
    return [UIColor colorWithRed:230.0/255.0 green:244.0/255.0 blue:254.0/255.0 alpha:1.0];
}

+ (UIColor *)tripTrunkBlueMapLine {
    return [UIColor colorWithRed:1.0/255.0 green:148.0/255.0 blue:254.0/255.0 alpha:1.0];
}

+ (UIColor *)onboardingButtonColorBlue{
    return [UIColor colorWithRed:82.0/255.0 green:183.0/255.0 blue:255.0/255.0 alpha:1.0];
}

//+ (UIColor *)tripTrunkCoral {
//    return [UIColor colorWithRed:217.0/255.0 green:135.0/255.0 blue:135.0/255.0 alpha:1.0];
//}

@end
